...
import { experienceData } from '../../themes/purple/experience-data'

const props = defineProps<{ lang: string }>()
const locale = computed(() => (props.lang === 'fr' || props.lang === 'lb' ? props.lang : 'en'))
const open = ref<number[]>(experienceData.items.map((_, i) => i))
...
<template>
  <div class="my-6 mb-8">
    ...
    >
      <span
        class="absolute -left-[9px] top-3 h-4 w-4 rounded-full border-2 border-brand-400 bg-surface dark:bg-brand-950"
      />
      <div class="flex gap-3">
        <span
          class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-brand-400/60 bg-brand-300/60 text-sm font-bold text-brand-900 dark:bg-brand-800/60 dark:text-fg"
        >
        ...
        <div class="min-w-0 flex-1 pb-6">
          ...
          >
            <span class="flex flex-col gap-0.5">
              <span class="text-lg font-semibold text-gray-900 dark:text-fg">{{ job.role[locale] }}</span>
